			...
			if(strcasecmp(cmd, "LIST") == 0)
			{
				if(data_s >= 0)
				{
					...
					void listcb(Lv2FsDirent entry)
					{
						...
						Lv2FsStat buf;
						if(lv2FsStat(filename, &buf)<0) return;
	...
	if(list_s > 0)
	{
		...
		while(exitapp == 0  || active_threads > 0)
		{
            ...
            sprintf(string, "%s%i", "ClientCmdHandler", active_threads);
				sys_ppu_thread_create(&id, handleclient, (u64)conn_s, 1000, 0x4000 + BUFFER_SIZE, 0, string);
...
int ftp_init()
{
	...
	sys_ppu_thread_create(&thread_id2, ipaddr_get, 0, 1000, 0x1000, 0, "RetrieveIPAddress");
    u64 retval;
    sys_ppu_thread_join(thread_id2, &retval);

	sys_ppu_thread_create(&thread_id, handleconnections, 0, 999, 0x1000, 0, "ServerConnectionHandler");
